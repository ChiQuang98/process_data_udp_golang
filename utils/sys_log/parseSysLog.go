...
func ParseSyslog(message []byte) string {
	...
	t1, err := time.Parse(layout1, result["timestamp"])
	msg := result["timestamp"] + "," + result["source-address"] + "," + result["source-port"] + "," + result["nat-destination-address"] + "," + result["nat-destination-port"]
	if err == nil {
		msg = t1.Format("20060102150215") + "," + result["source-address"] + "," + result["source-port"] + "," + result["nat-destination-address"] + "," + result["nat-destination-port"]

	}
	t2, err := time.Parse(layout2, result["timestamp"])
	if err == nil {
		msg = t2.Format("20060102150215") + "," + result["source-address"] + "," + result["source-port"] + "," + result["nat-destination-address"] + "," + result["nat-destination-port"]
	}
	return msg
}
